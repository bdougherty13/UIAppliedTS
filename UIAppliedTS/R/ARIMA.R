#' ARIMA
#' @export
#' @description This function is very simiar to the arima function in TSA and the arimax function which builds on and extends the capability of the arima function in R stats by allowing the incorporation of transfer functions, and innovative and additive outliers. It differentiates from arima allowing for the calculation of a drift term.
#' @param order regular ARIMA order
#' @param seasonal seasonal ARIMA order
#' @param xreg a dataframe containing covariates
#' @param included.mean Should the ARIMA model include a mean term? The default is TRUE for undifferenced series, FALSE for differenced ones (where a mean would not affect the fit nor predictions).
#' @param transform.pars if true, the AR parameters are transformed to ensure stationarity
#' @param fixed a vector indicating which coefficients are fixed or free
#' @param init initial values
#' @param method 	Fitting method: maximum likelihood or minimize conditional sum-of-squares. The default (unless there are missing values) is to use conditional-sum-of-squares to find starting values, then maximum likelihood.
#' @param n.cond number of initial values to be conditioned on in a conditional analysis
#' @param optim.control control parameters for the optimization procedure
#' @param kappa prior variance; used in dealing with initial values
#' @param . All of the above parameters have the same usage as those in the arima function. Please check the help manual of the arima function. Below are new options.
#' @param io a list of time points at which the model may have an innovative outlier. The time point of the outlier can be given either as absolute time point or as c(a,b), i.e. at the b-th 'month' of the a-th 'year' where each year has frequency(x) months, assuming x is a time series.
#' @param xtransf xtranf is a matrix with each column containing a covariate that affects the time series response in terms of an ARMA filter of order (p,q), i.e. if Z is one such covariate, its effect on the time series is $(theta_0+theta_1B+...+theta_{q-1}B^{q-1})/(1-phi_1 B -...-phi_p B^p) Z_t$ In particular, if $p=0$ and $q=1$, this specifies a simple regression relationship, which should be included in xreg and not here. Note that the filter starts with zero initial values. Hence, it is pertinent to mean-delete each distributed-lag covariate, and this is not done automatically.
#' @param transfer a list consisting of the ARMA orders for each transfer (distributed lag) covariate.
#' @param include.drift Should the ARIMA model include a linear drift term? (i.e., a linear regression with ARIMA errors is fitted.) The default is FALSE.
#' @param include.constant If TRUE, then include.mean is set to be TRUE for undifferenced series and include.drift is set to be TRUE for differenced series. Note that if there is more than one difference taken, no constant is included regardless of the value of this argument. This is deliberate as otherwise quadratic and higher order polynomial trends would be induced.
#' @param lambda Box-Cox transformation parameter. If lambda="auto", then a transformation is automatically selected using BoxCox.lambda. The transformation is ignored if NULL. Otherwise, data transformed before model is estimated.
#' @param biasadj Use adjusted back-transformed mean for Box-Cox transformations. If transformed data is used to produce forecasts and fitted values, a regular back transformation will result in median forecasts. If biasadj is TRUE, an adjustment will be made to produce mean forecasts and fitted values.
#' @param model Output from a previous call to Arima. If model is passed, this same model is fitted to y without re-estimating any parameters.
#' @author

ARIMA<-function (x, order = c(0, 0, 0), seasonal = list(order = c(0,0,0),
                                                        period = NA),
                 xreg = NULL, include.mean = TRUE, transform.pars = TRUE,
                 fixed = NULL, init = NULL, method = c("CSS-ML", "ML", "CSS"),
                 n.cond, optim.control = list(), kappa = 1e+06, io = NULL,
                 xtransf, transfer = NULL,include.drift=FALSE,include.constant,
                 lambda=NULL, biasadj = FALSE,model=NULL
                 )
{
  deriv = function(u) {
    p = length(u)
    ...
    for (i in 1:p) {
      ...
      for (k in 2:p) {
        ...
        mult1 = newderiv[k]
        for (j in 1:(k - 1)) {
          newphi[j] = oldphi[j] - mult * oldphi[k -
                                                  j]
  ...
  btrarma = function(par, arma, trans) {
    ...
    if (arma[4]) {
      ...
      Theta = Theta[-seq(length(Theta) - arma[5] + 2,
                         length(Theta))]
    }
    else Theta = 1
  ...
  armafn <- function(p, trans) {
    ...
    if (ntransf.term)
      x = upx(x, par)
    res = forecast::Arima(y=x, order = order, seasonal = seasonal,
                          fixed = par[1:narma], method = "ML", include.mean = FALSE,
                          include.drift = include.drift,include.constant = include.constant,
                          transform.pars = FALSE, lambda=lambda,
                          biasadj = biasadj,model=model,x=x)
    (-2 * res$loglik - n.used * (1 + log(2 * pi)))/(2 *
                                                      n.used)
  }
  armafn1 <- function(p, trans) {
    ...
    #############Changed forecast function below#######
    res = forecast::Arima(y=x, order = order, seasonal = seasonal,
                          fixed = par[1:narma], method = "ML", include.mean = FALSE,
                          include.drift = include.drift,include.constant = include.constant,
                          transform.pars = FALSE, lambda=lambda,biasadj = biasadj,model=model,x=x)
    res
  }
  armaCSS <- function(p, trans) {
    ...
    if (ntransf.term)
      x = upx(x, par)
    ###Changed stats:arima to forecast::Arima added include drift and include.constant
    res = forecast::Arima(y=x, order = order, seasonal = seasonal,
                          fixed = par[1:narma], method = "CSS", include.mean = FALSE,
                          include.drift = include.drift,include.constant = include.constant,
                          transform.pars = FALSE,lambda=lambda,biasadj = biasadj,model=model,x=x)$sigma2
    0.5 * log(res)
  }
  armaCSS1 <- function(p, trans) {
    ...
    if (ntransf.term)
      x = upx(x, par)
    ###Changed stats:arima to forecast::Arima added include drift and include.constant
    forecast::Arima(y=x, order = order, seasonal = seasonal,
                    fixed = par[1:narma], method = "CSS", include.mean = FALSE,
                    include.drift = include.drift,include.constant = include.constant,
                    transform.pars = FALSE,lambda=lambda,
                    biasadj = biasadj,model=model,x=x)
  ...
  if (missing(io) & missing(xtransf) & missing(transfer) &
    ...
    res = forecast::Arima(y = x, order = order, seasonal = seasonal,
                          ...
                          include.drift = include.drift,include.constant = include.constant,
                          optim.control = optim.control, kappa = kappa,lambda=lambda,
                          biasadj = biasadj,model=model,x=x)
    res$call = match.call()
    res$aic = res$aic
    return(res)
  ...
  if (method == "CSS") {
    ...
    if (transform.pars) {
      ...
      A <- A[mask, mask]
      var <- t(A) %*% solve(res$hessian * n.used) %*%
        A
